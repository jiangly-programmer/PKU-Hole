...
namespace {
string geturl(string website, string action, vector<std::pair<string, int>>param, string param2="") {
	string res = website + "?";
	res += "action=" + action;
	for (auto p : param) {
...
#define hex4(i) ((hex_to_int(s[i+2])<<12)|(hex_to_int(s[i+3])<<8)|(hex_to_int(s[i+4])<<4)|(hex_to_int(s[i+5])))
	string res;
	for (int i = 0; i < (int)s.size(); i++) {
		if (s[i] != '\\') {
			res.push_back(s[i]);
...
JSON HoleAPI::search(string word, int page) const {
	return getjson(geturl(CONFIG::url, "search", {{"page", page}}, "&keywords="+utf8_to_url(word)+"&pagesize=50"));
}
vector<JSON> HoleAPI::multi_getone(const vector<int> &pid) const {
	if (!pid.size()) return vector<JSON>();
	...
	for (int i = 0; i < (int)pid.size(); i++) {
		curl[i] = my_easy_init(geturl(CONFIG::url, "getone", {{"pid", pid[i]}}), res[i]);
	...
	vector<JSON>RES;
	for (auto s : res)
		if (s.size() && s.find("503 Service Temporarily Unavailable") == string::npos) {
...
vector<JSON> HoleAPI::multi_getcomment(const vector<int> &pid) const {
	...
	int running;
	for (int i = 0; i < (int)pid.size(); i++) {
		curl[i] = my_easy_init(geturl(CONFIG::url, "getcomment", {{"pid", pid[i]}}), res[i]);
		curl_multi_add_handle(multi, curl[i]);
	}
	for (int _ = 1; _ <= 3; _++) {
		clock_t _start = clock();
