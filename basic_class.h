...
struct Image {
	string url;
	inline explicit Image(const Image & x) : url(x.url) {
	}
	inline explicit Image(const string & x = "which sb do not fill url") : url(x) {
	}
...
struct Text {
	string s;
	set<string> words;
	inline Text(const Text & x) : s(x.s), words(x.words) {
	}
